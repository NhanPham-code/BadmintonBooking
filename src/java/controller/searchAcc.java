...
public class searchAcc extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet searchAcc</title>");  
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet searchAcc at " + request.getContextPath () + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    } 
    ...
    throws ServletException, IOException {
        ...
        if(email == null) {
            request.setAttribute("error", "Please enter email for searching");
        ...
        if(acc == null) {
            request.setAttribute("notify", "No result for seaching");
            request.getRequestDispatcher("view/admin/AdAccManage.jsp").forward(request, response);
        }else {
            accList.add(acc);
